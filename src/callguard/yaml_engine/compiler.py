...
def _compile_pre(contract: dict, mode: str) -> Any:
    ...
    # Stamp metadata so CallGuard can filter/route
    precondition_fn.__name__ = contract_id
    precondition_fn._callguard_type = "precondition"
    precondition_fn._callguard_tool = tool
...
def _compile_post(contract: dict, mode: str) -> Any:
    ...
    postcondition_fn._callguard_type = "postcondition"
    postcondition_fn._callguard_tool = tool
...
def _compile_session(contract: dict, mode: str, limits: OperationLimits) -> Any:
    ...
    async def session_contract_fn(session: Any) -> Verdict:
        from callguard.session import Session as _Session
    ...
    session_contract_fn._callguard_type = "session_contract"
    session_contract_fn._callguard_mode = mode
    session_contract_fn._callguard_id = contract_id
