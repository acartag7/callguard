"""CallGuard CLI — validate, check, diff, and replay contract bundles."""
...
def _load_and_compile(path: str) -> tuple[dict, str, Any]:
    ...
    Raises CallGuardConfigError on invalid bundles.
    """
...
def _evaluate_preconditions(
    ...
    for fn in compiled.preconditions:
        tool_filter = getattr(fn, "_callguard_tool", "*")
        if tool_filter != "*" and tool_filter != envelope.tool_name:
            continue

        verdict = fn(envelope)
        record = {
            "id": getattr(fn, "_callguard_id", "unknown"),
...
def validate(files: tuple[str, ...]) -> None:
    ...
    for file_path in files:
        ...
        except CallGuardConfigError as e:
