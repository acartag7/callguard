...
CallGuard provides runtime governance for AI agent tool calls. This page maps
CallGuard features to specific requirements in the EU AI Act and SOC 2 Trust
...
| Requirement | CallGuard Feature | Evidence / Configuration |
|-------------|-------------------|------------------------|
| Risk identification | Preconditions define what constitutes a risky tool call. Each contract names a specific risk (e.g. `no-destructive-bash`, `no-secrets-in-args`). | YAML contract file listing all governed risks. `callguard validate` confirms contracts are well-formed. |
...
| Requirement | CallGuard Feature | Evidence / Configuration |
|-------------|-------------------|------------------------|
| Understand capacities and limitations | Contract files are human-readable YAML that declare exactly what the agent is and is not allowed to do. `callguard check` lets a human test any hypothetical tool call without running it. | YAML contract file reviewed in code review. `callguard check` output in documentation or runbooks. |
| Monitor operation | Audit sinks stream every governance decision to observability platforms in real time. OpenTelemetry spans show tool calls in distributed traces. | `StdoutAuditSink`, `FileAuditSink`, `WebhookAuditSink`, `SplunkHECSink`, `DatadogSink` configured and emitting. OTel `callguard.calls.denied` / `callguard.calls.allowed` counters on dashboards. |
...
| Ability to override or reverse | Per-contract observe mode lets operators disable enforcement on individual rules without a full redeploy. `callguard replay` lets operators preview the impact before re-enabling enforcement. | Per-contract `mode: observe` in YAML. `callguard replay --only-changes` output in change management ticket. |
...
| Protection of sensitive data | `deny_sensitive_reads()` built-in blocks access to common secret paths (`~/.ssh/`, `.env`, `/var/run/secrets/`, AWS credentials). `RedactionPolicy` scrubs sensitive data from audit payloads. | `deny_sensitive_reads()` registered in CallGuard. `RedactionPolicy` configured on all audit sinks. |
...
| Change impact analysis | `callguard diff` compares two contract files and reports added, removed, and changed contract IDs. `callguard replay` re-evaluates historical audit events against a proposed contract file and reports what would change. | `callguard diff old.yaml new.yaml` in CI on every PR. `callguard replay --contracts new.yaml --audit-log prod.jsonl --only-changes` in change approval workflow. |
...
| `callguard validate` output in CI | Art. 9 (risk identification) | CC6.8 (testing) |
| `callguard diff` output in PR review | Art. 9 (lifecycle documentation) | CC6.8 (change impact) |
| `callguard replay` report | Art. 9 (risk estimation), Art. 14 (override preview) | CC6.8 (change impact analysis) |
